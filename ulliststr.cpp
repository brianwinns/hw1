...
{
  //check if list is empty or if tail node is full at the back
  if (tail_ == NULL || tail_->last == ARRSIZE){ 
    //if empty or full at back, allocate new tail node
    Item* newNode = new Item();
    newNode->val[0] = value;
    newNode->last = 1;
    
    //if there is a current tail node (full tail), link the new tail node
    if (tail_ != NULL){
      tail_->next = newNode;
      newNode->prev = tail_;
    }

    //set tail to the new tail node
    tail_ = newNode;
    //if list was previously empty, set head and tail equal to this new, singular node
    if (head_ == NULL){
      head_ = tail_;
    }
  }

  //if tail node has space, push new value to back of tail node
  else {
...
std::string const & ULListStr::back() const
{
  ...
  if (ULListStr::tail_ == NULL){
    throw std::out_of_range("List is empty");
  }

  //return last element
  return ULListStr::tail_->val[tail_->last-1];
}
...
std::string const & ULListStr::front() const
{
  ...
  return ULListStr::head_->val[head_->first];
}
...
std::string* ULListStr::getValAtLoc(size_t loc) const
{
  ...
  //occurs if something element not found (error case)
  return NULL;
}
